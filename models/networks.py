...
# adaptsegnet model
################################################
class Classifier_Module_Mul(nn.Module):
    def __init__(self, inplanes, dilation_series, padding_series, num_classes):
        super(Classifier_Module_Mul, self).__init__()
        self.conv2d_list = nn.ModuleList()
        for dilation, padding in zip(dilation_series, padding_series):
...
class ResNetMulti(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        self.layer3 = self._make_layer(block, 256, layers[2], stride=1, dilation=2)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=1, dilation=4)
        self.layer5 = self._make_pred_layer(Classifier_Module_Mul, 1024, [6, 12, 18, 24], [6, 12, 18, 24], num_classes)
        self.layer6 = self._make_pred_layer(Classifier_Module_Mul, 2048, [6, 12, 18, 24], [6, 12, 18, 24], num_classes)
    ...
    def forward(self, x):
        ...
        x2 = self.layer4(x)
        x2 = self.layer6(x2)
        x1 = nn.functional.interpolate(x1, size=(h, w), mode='bilinear')
        x2 = nn.functional.interpolate(x2, size=(h, w), mode='bilinear')
    ...
    def optim_parameters(self, opt):
        return [{'params': self.get_1x_lr_params_NOscale(), 'lr': opt.lr},
                {'params': self.get_10x_lr_params(), 'lr': 10 * opt.lr}]
def AdaptSegnet(is_restore_from_imagenet=True, resnet_weight_path="./resnetweight/",  num_classes=2, gpu_ids=[]):
