...
def save_segment_result(visuals, epoch, win_size, img_dir, web_dir, name):
    # save images to the disk
...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        if self.use_html:  # create an HTML object at <checkpoints_dir>/web/; images will be saved under <checkpoints_dir>/web/images/
            self.web_dir = os.path.join(opt.checkpoints_dir, opt.name, 'train')
    ...
    def display_current_results_segment(self, visuals, epoch):
        """save current results to an HTML file.
    ...
    def print_current_losses(self, epoch, iters, losses, t_comp):
        ...
        """
        message = '(epoch: %d, iters: %d, run_one_iter_time: %.3f) ' % (epoch, iters, t_comp)
